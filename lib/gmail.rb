...
module Gmail
  class << self
    ...
    def new(hash)
      %i[auth_method client_id client_secret refresh_token auth_scopes email_account application_name application_version].each do |accessor|
  ...
  def self.request(method, params = {}, body = {}, auth_method = @auth_method)
    ...
    when 'service_account'
      if @client.nil?
        service_account_connect
      elsif client.authorization.principal != @email_account
        service_account_connect
      end
    end
  ...
  def self.mailbox_email
    @mailbox_email ||= request(@service.users.to_h['gmail.users.getProfile'])[:emailAddress]
  end

  def self.connect(client_id = @client_id, client_secret = @client_secret, refresh_token = @refresh_token)
    raise 'No client_id specified' unless client_id

    raise 'No client_secret specified' unless client_secret

    raise 'No refresh_token specified' unless refresh_token
