  ...
  class MessageTest < Test::Unit::TestCase
    ...
    should 'message should be retrievable by id' do
      @mock.expects(:execute).with(
        ...
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_message))
    ...
    should 'message construct should set some basics values' do
      ...
      ['From', 'To', 'Cc', 'Subject', 'Bcc', 'Date', 'Message-ID', 'References', 'In-Reply-To', 'Delivered-To'].each do |method|
        assert_equal test_message[:payload][:headers]
                       .select { |h| h[:name].casecmp(method.downcase).zero? }
                       .first[:value], m.send(method.downcase.tr('-', '_'))
      end
    ...
    should 'Access thread from message' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.threads.get,
        parameters: {
          userId: 'me',
          id: test_message[:threadId]
        },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_thread))
      m = Gmail::Message.new(test_message)
      t = m.thread
    ...
    should 'message should be deletable' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.messages.delete,
        parameters: {
          userId: 'me',
          id: test_message[:id]
        },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(''))
    ...
    should 'message should be thrashable' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.messages.trash,
        parameters: {
          userId: 'me',
          id: test_message[:id]
        },
    ...
    should 'message should be unthrashable' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.messages.untrash,
        parameters: {
          userId: 'me',
          id: test_message[:id]
        },
    ...
    context 'Modifying Labels' do
      should 'message should be starrable' do
        @mock.expects(:execute).with(
          api_method: Gmail.service.users.messages.modify,
          parameters: {
            userId: 'me',
            id: test_message[:id]
          },
      ...
      should 'message label should be modifiable as wish' do
        @mock.expects(:execute).with(
          ...
          body_object: {
            addLabelIds: ['UNREAD', 'SOME COOL LABEL'],
            removeLabelIds: ['INBOX', 'SOME NOT COOL LABEL']
          },
          headers: { 'Content-Type' => 'application/json' }
        ).twice.returns(test_response(test_message))
        t = Gmail::Message.new(test_message)
    ...
    should 'Message should be searcheable' do
      @mock.expects(:execute).with(
        ...
        parameters: {
          userId: 'me',
          q: 'from:(me) to:(you) subject:(subject) in:inbox before:2014/12/1 after:2014/11/1 test -{real}',
          labelIds: ['UNREAD']
        },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_message_list))
      list = Gmail::Message.search(
        from: 'me',
        to: 'you',
        subject: 'subject',
        in: 'inbox',
    ...
    should 'Draft can be created from Message' do
      ...
      ###
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.create,
        parameters: {
          userId: 'me'
        },
        body_object: {
          message: {
            raw: m.raw,
            threadId: test_message[:threadId],
            labelIds: test_message[:labelIds]
          }
        },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_draft))
      d = m.create_draft
    ...
    should 'Message should be sendable and return a Message' do
      ...
      m.raw = m.raw
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.messages.to_h['gmail.users.messages.send'],
    ...
    should 'Reply to all construct should be easy and call getProfile if delivered_to is not set' do
      ...
      reply_message = Gmail::Message.new test_reply_message
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.to_h['gmail.users.getProfile'],
        parameters: { userId: 'me' },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(emailAddress: 'julie@juliedesk.com'))
      new_m = m.reply_all_with reply_message
      expected_msg = Gmail::Message.new test_replied_message
    ...
    should 'forward construct should be easy' do
      ...
      forward_message = Gmail::Message.new(
        to: 'test@test.com',
        bbc: 'coucou',
        cc: 'test@couocu.com, second@second.com',
    ...
    should 'Insert call should be easy' do
      ...
      m.raw = m.raw
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.messages.insert,
        parameters: {
          userId: 'me'
        },
        body_object: {
