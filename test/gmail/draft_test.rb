...
module Gmail
  class DraftTest < Test::Unit::TestCase
    should 'Draft should be retrievable by id' do
      @mock.expects(:execute).with(api_method: Gmail.service.users.drafts.get,
        parameters: {
          userId: 'me',
          id: test_draft[:id] },
      ...
      assert d.is_a? Gmail::Draft
      assert_equal test_draft[:id], d.id
    end

    should 'drafts should be listable' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.list,
        parameters: { userId: 'me' },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_draft_list))
      list = Gmail::Draft.all
      assert list.is_a? Array
      assert list[0].is_a? Gmail::Draft
    end
    ...
    context 'Message Object in draft' do
      should 'retrieved Draft should not generate call to get Message Object' do
        ...
        assert draft.message.is_a? Gmail::Message
      end
      ...
      should 'Draft get from a draft list should generate call to get Message Object' do
        ...
        @mock.expects(:execute).with(
          api_method: Gmail.service.users.drafts.get,
          parameters: {
            userId: 'me',
            id: test_draft[:id]
          },
    ...
    should 'drafts should be deletable' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.delete,
        parameters: {
          userId: 'me',
          id: test_draft[:id]
        },
    ...
    should 'drafts should be updateable' do
      ...
      draft_hash[:message][:labelIds] = ['COOL LABEL']
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.get,
        parameters: {
          id: test_draft[:id],
        ...
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(draft_hash))
      ...
      draft_hash[:message][:labelIds] = ['INBOX']

      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.update,
        parameters: {
          id: test_draft[:id],
          userId: 'me'
        },
        body_object: {
          message: {
            raw: d.message.raw,
            threadId: test_draft[:message][:threadId],
            labelIds: ['INBOX']
          }
        },
        headers: { 'Content-Type' => 'application/json' }
      ).twice.returns(test_response(draft_hash))
    ...
    should 'create should return a new Draft' do
      ...
      @mock.expects(:execute).with(
        ...
        body_object: {
          message: {
            raw: d.message.raw,
            threadId: draft_hash[:message][:threadId],
            labelIds: draft_hash[:message][:labelIds]
          }
        },
        headers: { 'Content-Type' => 'application/json' }
      ).once.returns(test_response(test_draft))
    ...
    should 'Draft should be sendable and return a Message' do
      @mock.expects(:execute).with(
        api_method: Gmail.service.users.drafts.to_h['gmail.users.drafts.send'],
        parameters: { userId: 'me' },
        body_object: { id: test_draft[:id] },
        headers: { 'Content-Type' => 'application/json' }
