    ...
    module List
      module ClassMethods
        def all(filters = {}, opts = {})
          ...
          filters[:maxResults] = 100 if max_results == -1
        ...
        def search(q = {})
          if q.is_a? String
            ...
            query = ''
            %i[from to subject].each do |prop|
              query += "#{prop}:(#{q[prop].downcase}) " unless q[prop].nil?
              q.delete(prop)
            end
            %i[in before after].each do |prop|
              query += "#{prop}:#{q[prop]} " unless q[prop].nil?
              q.delete(prop)
            end
